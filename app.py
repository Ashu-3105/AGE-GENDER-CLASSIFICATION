...
if(app_mode=="Home"):
    ...
    if(st.button("Predict")):
        ...
        if age_pred>30 and age_pred<50:
            st.text(f"Predicted Gender: {gender_pred} and Age range is: 25-40 ")
        elif age_pred>60 and age_pred<80:
            st.text(f"Predicted Gender: {gender_pred} and Age is: {age_pred} ")
        elif age_pred>2 and age_pred<20:
            st.text(f"Predicted Gender: {gender_pred} and Age range is: 2-20  ")
        else:
           st.text(f"Predicted Gender: {gender_pred} and Age is: 20-30 ")
